...
def main():
    ...
    if not os.path.exists(DOCS_DIR):
        ...
        print("Please place your PDF files inside this folder and run the script again.")
        sys.exit()
        
    if not os.path.exists(MODEL_DIR):
        os.makedirs(MODEL_DIR)
        print(f"Created directory '{MODEL_DIR}'.")
        print("Please download a Llama 3.2 GGUF model file, name it 'llama-3.2.gguf', place it in the models folder, and run again.")
        sys.exit()
        
    if not os.path.exists(MODEL_PATH):
        print(f"Model file not found at '{MODEL_PATH}'.")
        print("Please download a Llama 3.2 GGUF file and place it in the models folder.")
        sys.exit()
    ...
    print("Generating embeddings (on the first run, this will download the embedding model).")
